...
namespace UnitySlippyMap.Helpers
{
    ...
    public class GeoHelpers
    {
        ...
        /// <summary>
        /// Returns WGS84 given ScreenPotin 
        /// </summary>
        /// <param name="map"></param>
        /// <param name="pos"></param>
        /// <returns></returns>
        public static double[] ScreenpointToWGS84(MapBehaviour map,Vector3 pos)
        {
            double[] DoubleToEPSG900913 = new double[] { (map.CenterEPSG900913[0]) + (pos.x / map.ScaleMultiplier), (map.CenterEPSG900913[1]) + (pos.z / map.ScaleMultiplier) };
            return map.EPSG900913ToWGS84Transform.Transform(DoubleToEPSG900913);
        }
        /// <summary>
        /// Returns RaycastHit given WGS84
        /// </summary>
        /// <param name="map"></param>
        /// <param name="wgs"></param>
        /// <returns></returns>
        public static double[] WGS84ToRaycastHit(MapBehaviour map,double[] wgs)
        {
            double[] temp = map.WGS84ToEPSG900913Transform.Transform(wgs);
            double[] EPSG900913ToRaycastHit = new double[] { (temp[0] - map.CenterEPSG900913[0]) * map.ScaleMultiplier, (temp[1] - map.CenterEPSG900913[1]) * map.ScaleMultiplier };
            return EPSG900913ToRaycastHit;
        }
    }
}
